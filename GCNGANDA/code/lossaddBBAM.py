...
def bbam_loss_function(pred, target, s=30.0, m=0.35):
    ...
    简化版的 BBAM 损失：
      ...
      这里先计算转换后的余弦值，并用 sigmoid 模拟 BBAM 损失计算。
...
for epoch in range(1, n_epochs + 1):
    ...
    # BBAM 损失：这里仅在有标签数据上计算
    bbam_loss_val = bbam_loss_function(output_train[train_mask], Y_train_tensor[train_mask])

    # 总损失：监督损失 + λ_align * 分布对齐损失 + λ_bbam * BBAM 损失
    total_loss = sup_loss + lambda_align * align_loss + lambda_bbam * bbam_loss_val
    ...
    if epoch % 10 == 0:
        ...
        with torch.no_grad():
            ...
            print(
                f"Epoch {epoch:03d}, Loss: {total_loss.item():.4f}, Sup Loss: {sup_loss.item():.4f}, Align Loss: {align_loss.item():.4f}, BBAM Loss: {bbam_loss_val.item():.4f}, 训练集有标签准确率: {train_sup_acc:.4f}")
