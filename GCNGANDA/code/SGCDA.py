...
############################################
# 4. 定义 SGC 模型（简化图卷积网络，适应多标签任务）
############################################
# SGC 的核心思想是用 (A_norm)^K 预传播特征后，再用一个线性层进行分类
class SGC(nn.Module):
    def __init__(self, nfeat, nclass, K=2, dropout=0.5):
        """
        参数：
          nfeat: 输入特征维度
          nclass: 类别数
          K: 特征传播次数（即 A_norm 的幂次数）
          dropout: dropout 概率
        """
...
model = SGC(nfeat=n_features, nclass=n_classes, K=2, dropout=0.5)
