...
############################################
# 4. 定义改进后的 GCN 模型（适应多标签任务，同时尝试提高准确率）
############################################
class GCNLayer(nn.Module):
    ...
    def forward(self, x, A_norm):
        out = self.linear(x)
        out = torch.mm(A_norm, out)
        return out


class GCNImproved(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout=0.5):
        super(GCNImproved, self).__init__()
        # 提高隐藏层维度
        self.gc1 = GCNLayer(nfeat, nhid)
        self.bn1 = nn.BatchNorm1d(nhid)
        self.gc2 = GCNLayer(nhid, nclass)
        self.dropout = dropout
        # 如果输入和隐藏层维度相同，可考虑加入残差连接
        if nfeat == nhid:
            self.residual = True
        else:
            self.residual = False
    ...
    def forward(self, x, A_norm):
        ...
        out2 = self.gc2(out1, A_norm)
        # 输出 raw logits
        return out2


n_features = X_train_tensor.shape[1]
# 这里将隐藏层维度调为32（可根据需要调整）
model = GCNImproved(nfeat=n_features, nhid=32, nclass=n_classes, dropout=0.5)
# 使用较小学习率，并增加权重衰减来防止过拟合
