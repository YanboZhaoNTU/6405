...
class FocalLoss(nn.Module):
    ...
    def forward(self, output_logits, target):
        # 使用 F.cross_entropy 得到每个样本的 loss，然后计算 focal loss
...
###############################################################################
# 自定义数据集（如果标签为 one-hot 则转换为类别索引）
###############################################################################
class CustomDataset(Dataset):
    def __init__(self, X, Y):
        self.X = X  # 假设 X 为列表或 numpy 数组
        self.Y = Y  # 如果 Y 是 one-hot 编码，则需转换为类别索引
...
# 这里用 np.unique 获取标签数，如果标签已经转换为索引
...
# 训练设置与选择损失函数
###############################################################################
# 选择损失函数类型："focal", "ce", 或 "ldam"
loss_type = "ce"  # 修改此处以选择不同的损失函数
...
if loss_type == "focal":
    ...
    cls_num_list = [50, 100, 200, 300, 400]  # 示例列表，请根据实际数据调整
