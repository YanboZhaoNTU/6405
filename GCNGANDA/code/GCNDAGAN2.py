...
# 待测试的数据集名称列表
datasnames = [
    "20NG", "3Sources_bbc1000", "3Sources_guardian1000", "3Sources_inter3000", "3Sources_reuters1000",
    "Bibtex", "Birds", "Bookmarks", "CAL500", "CHD_49",
    "Corel16k001", "Corel16k002", "Corel16k003", "Corel16k004", "Corel16k005",
    "Corel16k006", "Corel16k007", "Corel16k008", "Corel16k009", "Corel16k010",
    "Corel5k", "Delicious", "Emotions", "Enron", "EukaryoteGO",
    "EukaryotePseAAC", "Eurlex_dc_1", "Eurlex_ev_1", "Eurlex_sm_1", "Flags",
    "Foodtruck", "Genbase", "GnegativeGO", "GnegativePseAAC", "GpositiveGO",
    "GpositivePseAAC", "HumanGO", "HumanPseAAC", "Image", "Imdb",
    "Langlog", "Mediamill", "Medical", "Ohsumed", "Nuswide_BoW",
    "Nuswide_cVLAD", "PlantGO", "PlantPseAAC", "Rcv1subset1", "Rcv1subset2",
    "Rcv1subset3", "Rcv1subset4", "Rcv1subset5", "Reuters_K500", "Scene",
    "Slashdot", "Chemistry", "Chess", "Coffee", "Cooking",
    "CS", "Philosophy", "Tmc2007", "Tmc2007_500", "VirusGO",
    "VirusPseAAC", "Water_quality", "Arts", "Business", "Computers",
    "Education", "Entertainment", "Health", "Recreation", "Reference",
    "Science", "Social", "Society", "Yeast", "Yelp"
]

# 用于存放各个数据集的评估结果，后面用于生成合并 Excel 文件
all_results = {}

# 外层循环：遍历每个数据集（每次只处理一个数据集，新建 ReadData 对象）
for dataset in datasnames:
    print("==============================================")
    print(f"开始处理数据集: {dataset}")

    # 每次构造的 ReadData 对象仅包含当前数据集名称
    rd = ReadData(datas=[dataset], genpath='data/')
    # 由于内部列表只有一个元素，索引固定为 0
    ...
    # 定义额外模块：伪标签生成器、编码器、样本生成器、SCM先验和判别器
    ...
    model_G = SampleGenerator(label_dim=n_classes, latent_dim=latent_dim, hidden_dim=hidden_dim, out_features=in_feat).to(device)
...
# 10. 汇总所有数据集的评估结果到一个 Excel 文件，每个数据集为一个工作表
