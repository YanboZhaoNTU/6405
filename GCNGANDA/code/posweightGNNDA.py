...
############################################
# 5. 定义多标签损失函数及分布对齐损失（针对长尾问题的改进）
############################################
# 针对长尾问题，我们根据训练数据统计各类别正样本数量，并计算 pos_weight
epsilon = 1e-6
# 统计每个类别正样本数（假设标签为0/1）
class_counts = Y_train_tensor.sum(dim=0)
# pos_weight 定义为 (负样本数/正样本数)，负样本数 = 总样本数 - 正样本数
pos_weight = (n_train - class_counts) / (class_counts + epsilon)
print("每个类别的 pos_weight:", pos_weight)

# 使用加权的 BCEWithLogitsLoss 来缓解长尾问题
